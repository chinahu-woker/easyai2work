...
<div >
    ...
    <view class="banner-wrap" @click="navigateToFirstApp">
        <div class="inner-row">
            <div class="left-card">
                <div class="left-card-inner">
                    <div class="left-texts">
                        ...
                        <div class="btns">
                            <button class="pill primary text-2xl text-center align-middle" @click="onPrimaryClick">
                              {{ primaryText }}
                            </button>
                        </div>
                    </div>
                ...
                <div class="right-inner">
                    <div class="texts">
                        ...
                        <div class="right-desc">{{ rightDesc }}</div>
                         <div class="btns">
                            <button class="pill secondary" @click="onSecondaryClick">{{ rightButtonText }}</button>
                        </div>
                    </div>
...
const navigateToFirstApp = () => {
    ...
    // 如果没有与 title 匹配的，尝试使用 rightTitle 筛选的应用作为回退
...
function onSecondaryClick() { 
  // 对于右侧按钮，可以基于 rightTitle 进行筛选跳转
    if (rightMatchingApps.value && rightMatchingApps.value.length > 0) {
        uni.navigateTo({ url: `/pages/draw/apps/apps?id=${rightMatchingApps.value[0]._id}` })
        } else {
            uni.showToast({ title: '功能未开放', icon: 'none' })
        }
  emit('secondary') 
