...
onReady(()=>{
  ...
  socketInit({
    ...
    onMessage:(msg)=>{
      console.log('handle pay success message', msg);
      const msgObj = parseJSONToObject<{type:string,order_id:string}>(msg)
      if (!msgObj) {
        // 非 JSON 消息（如心跳 ping/pong 或其他控制信息），忽略
        console.log('PaymentPopup: received non-JSON or ignored message, skipping', msg);
        return;
      }
      const {type} = msgObj
      if(type === 'pay_success'){
