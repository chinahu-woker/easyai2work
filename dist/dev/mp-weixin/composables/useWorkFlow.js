...
function useWorkFlow() {
  ...
  const socketInit = async (options = {
    ...
    common_vendor.index.__f__("log", "at composables/useWorkFlow.ts:153", "socket init execution，status", (_a = socketState.socket) == null ? void 0 : _a.readyState);
    if ((socketState == null ? void 0 : socketState.isInitialized) && ((_c = (_b = socketState.options) == null ? void 0 : _b.params) == null ? void 0 : _c.type) === ((_d = options == null ? void 0 : options.params) == null ? void 0 : _d.type)) {
      common_vendor.index.__f__("log", "at composables/useWorkFlow.ts:155", "WebSocket is already initialized");
      return;
    }
    if ((socketState == null ? void 0 : socketState.isInitialized) && ((_f = (_e = socketState == null ? void 0 : socketState.options) == null ? void 0 : _e.params) == null ? void 0 : _f.type) !== ((_g = options == null ? void 0 : options.params) == null ? void 0 : _g.type)) {
      common_vendor.index.__f__("log", "at composables/useWorkFlow.ts:160", "WebSocket is already initialized,but scene is different,reinitialize");
    ...
    socketState.socket = common_vendor.index.connectSocket({
      url,
      complete: () => {
        common_vendor.index.__f__("log", "at composables/useWorkFlow.ts:173", "WebSocket connect complete");
      }
    });
    socketState.isInitialized = true;
    common_vendor.index.onSocketOpen((result) => {
      common_vendor.index.__f__("log", "at composables/useWorkFlow.ts:178", "WebSocket opened", result);
    ...
    common_vendor.index.onSocketError((err) => {
      common_vendor.index.__f__("error", "at composables/useWorkFlow.ts:192", "WebSocket onError", err);
      socketState.isInitialized = false;
    ...
    common_vendor.index.onSocketClose(() => {
      socketState.isInitialized = false;
      common_vendor.index.__f__("log", "at composables/useWorkFlow.ts:200", "WebSocket onClose");
  ...
  const handleSocketHandShake = (timeout = 200) => {
    return new Promise((resolve) => {
      ...
      (_a = socketState.socket) == null ? void 0 : _a.send({
        data: "ping",
        success(result) {
          common_vendor.index.__f__("log", "at composables/useWorkFlow.ts:220", result);
  ...
  const handleSocketMessage = (msg, callback) => {
    var _a, _b;
    common_vendor.index.__f__("log", "at composables/useWorkFlow.ts:275", "原始消息", msg);
    ...
    try {
      ...
      common_vendor.index.__f__("error", "at composables/useWorkFlow.ts:284", "解析WebSocket消息时出错，消息不是有效的JSON格式:", error);
      return;
    }
    if (!msgObj)
      return;
  ...
  const handleSubmitTaskTask = async () => {
    ...
    common_vendor.index.__f__("log", "at composables/useWorkFlow.ts:383", "newTask", newTask);
    if (!newTask) {
      return;
    }
