...
const _sfc_main = {
  ...
  methods: {
    ...
    open(options) {
      ...
      this.isCancelToast = options.isCancelToast === void 0 ? true : options.isCancelToast;
      this.isConfirmToast = options.isConfirmToast === void 0 ? true : options.isConfirmToast;
    ...
    initCanvas() {
      ...
      this.ctx_bg = common_vendor.index.createCanvasContext("chj_imgEdit_canvas_bg", this);
      this.ctx_hx = common_vendor.index.createCanvasContext("chj_imgEdit_canvas_hx", this);
      this.ctx_text = common_vendor.index.createCanvasContext("chj_imgEdit_canvas_text", this);
      this.ctx_save = common_vendor.index.createCanvasContext("chj_imgEdit_canvas_save", this);
      this.ctx_cj = common_vendor.index.createCanvasContext("chj_imgEdit_canvas_cj", this);
      this.ctx.clearRect(0, 0, 1e4, 1e4);
      this.ctx_bg.clearRect(0, 0, 1e4, 1e4);
      this.ctx_hx.clearRect(0, 0, 1e4, 1e4);
    ...
    drawRotatedImage() {
      ...
      this.$nextTick(async () => {
        ...
        if (this.degrees == 90 || this.degrees == 270) {
          ...
          const { width: canvasWidth2, height: canvasHeight2 } = await this.getNodeInfo("#chj_imgEdit_canvas_bg");
          let dx2, dy2, dWidth, dHeight;
          if (width >= height) {
            dy2 = 0;
            dHeight = canvasHeight2;
            dWidth = canvasHeight2 / width * height;
            dx2 = (canvasWidth2 - dWidth) / 2;
          } else {
            dx2 = 0;
            dWidth = canvasWidth2;
            dHeight = canvasWidth2 / height * width;
            dy2 = (canvasHeight2 - dHeight) / 2;
          }
          this.bgPosition = {
            x: dx2,
            y: dy2,
    ...
    touchstart(e) {
      ...
      if (e.touches.length === 2 && this.tx_list_activate != "裁剪") {
        ...
        getCenter(initialTouches[0], initialTouches[1]);
        isTowTouch = true;
        return;
      }
    ...
    // 手指触摸后移动
    touchmove(e) {
      if (e.touches[0].x == void 0 && e.touches[0].y == void 0) {
      ...
      if (isTowTouch && this.tx_list_activate != "裁剪") {
        ...
        if (e.touches.length === 2) {
          if (e.touches[1].x == void 0 && e.touches[1].y == void 0) {
    ...
    touchend(e) {
      ...
      if (e.changedTouches[0].x == void 0 && e.changedTouches[0].y == void 0) {
    ...
    // 画矩形
    drawRect({ x, y }) {
      if (this.tx_list_activate != "矩形" || this.tx_type_activate == "橡皮")
        return;
      let width, heigth;
      let dx, dy;
      if (this.moveTo.x > x) {
    ...
    // 画圆
    drawCircle({ x, y }) {
      if (this.tx_list_activate != "圆形" || this.tx_type_activate == "橡皮")
        return;
      let width, heigth;
      let dx, dy;
      if (this.moveTo.x > x) {
    ...
    drawHorizontalLine({ x, y }) {
      if (this.tx_list_activate != "横线" || this.tx_type_activate == "橡皮")
        return;
      this.setCanvasStyle();
      this.ctx.moveTo(this.moveTo.x, this.moveTo.y);
      this.ctx.lineTo(x, y);
    ...
    // 画箭头
    drawAarrow({ x, y }) {
      if (this.tx_list_activate != "箭头" || this.tx_type_activate == "橡皮")
        return;
      this.setCanvasStyle();
      this.ctx.moveTo(this.moveTo.x, this.moveTo.y);
      this.ctx.lineTo(x, y);
      const angle = Math.atan2(y - this.moveTo.y, x - this.moveTo.x);
    ...
    // 点击范围内的文字操作
    isInAreaClick({ x, y }) {
      if (this.tx_list_activate != "文本" || this.tx_type_activate == "橡皮")
        return;
      for (let index in this.textList) {
        ...
        if (item.isActive && x >= arcX - item.arcSize && x <= arcX - item.arcSize + item.arcSize * 2 && y >= arcY - item.arcSize && y <= arcY - item.arcSize + item.arcSize * 2) {
          this.textList.splice(index, 1);
          this.drawText();
          return;
        }
        if (x >= item.x - item.width / 2 && x <= item.x - item.width / 2 + item.width && y >= item.y - item.height / 2 && y <= item.y - item.height / 2 + item.height) {
          if (item.isActive) {
            this.text = item.text;
            const time = setTimeout(() => {
              this.textFocus = true;
    ...
    isInAreaMove({ x, y }) {
      ...
      for (let index in this.textList) {
        const item = this.textList[index];
        if (item.isActive && x >= item.x - item.width / 2 && x <= item.x - item.width / 2 + item.width && y >= item.y - item.height / 2 && y <= item.y - item.height / 2 + item.height) {
    ...
    changeColor(item) {
      ...
      if (item.type == "选择")
        return this.$refs["chj-color-picker"].open();
    ...
    historyEvent(type) {
      if (type == "回退") {
        ...
        if (this.history_index >= this.history_list.length - 1)
          return;
        this.history_index += 1;
        this.drawBgCanvas();
      } else if (type == "原图") {
...
if (!Array) {
  const _easycom_chj_color_picker2 = common_vendor.resolveComponent("chj-color-picker");
  _easycom_chj_color_picker2();
}
const _easycom_chj_color_picker = () => "../chj-color-picker/chj-color-picker.js";
if (!Math) {
  _easycom_chj_color_picker();
}
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return {
    a: $data.iconPath.positionReset,
    b: common_vendor.o((...args) => $options.initScaleAndTranslate && $options.initScaleAndTranslate(...args)),
    c: $data.history_index < 0,
    d: $data.iconPath.retreat_inactive,
    e: !($data.history_index < 0),
    f: $data.iconPath.retreat_active,
    g: common_vendor.o(($event) => $options.historyEvent("回退")),
    h: $data.history_index >= $data.history_list.length - 1,
    i: $data.iconPath.goForward_inactive,
    j: !($data.history_index >= $data.history_list.length - 1),
    k: $data.iconPath.goForward_active,
    l: common_vendor.o(($event) => $options.historyEvent("前进")),
    m: $data.iconPath.reset,
    n: common_vendor.o(($event) => $options.historyEvent("原图")),
    o: common_vendor.s(`transform:translate(${$data.x}px, ${$data.y}px) scale(${$data.scale})`),
    p: common_vendor.s(`transform:translate(${$data.x}px, ${$data.y}px) scale(${$data.scale})`),
    q: common_vendor.s(`transform:translate(${$data.x}px, ${$data.y}px) scale(${$data.scale})`),
    ...
    w: common_vendor.s(`transform:translate(${$data.x}px, ${$data.y}px) scale(${$data.scale})`),
    x: common_vendor.s(`transform:translate(${$data.x}px, ${$data.y}px) scale(${$data.scale})`),
    y: common_vendor.f($data.color_list, (i, index, i0) => {
      return {
        a: index,
        b: i.type == "选择" && !i.color ? 1 : "",
        c: i.color,
        d: i.id == $data.active_color_id ? $props.activeColor : "transparent",
        e: common_vendor.o(($event) => $options.changeColor(i), index)
      };
    }),
    z: $data.iconPath.close,
    A: common_vendor.o((...args) => $options.cancel && $options.cancel(...args)),
    B: common_vendor.t($options.active_label),
    C: $data.iconPath.confirm,
    D: common_vendor.o((...args) => $options.confirm && $options.confirm(...args)),
    E: $data.iconPath.pen,
    F: common_vendor.s($data.tx_type_activate === "笔" ? `background-color:${$props.activeColor}` : ""),
    G: common_vendor.o(($event) => $options.changeType("笔")),
    H: $data.iconPath.rubber,
    I: common_vendor.s($data.tx_type_activate === "橡皮" ? `background-color:${$props.activeColor}` : ""),
    J: common_vendor.o(($event) => $options.changeType("橡皮")),
    K: $data.pen_size,
    L: $props.activeColor,
    M: $props.activeColor,
    ...
    P: common_vendor.f($data.tx_list, (i, index, i0) => {
      return {
        a: i.icon,
        b: index,
        c: common_vendor.s(i.name == $data.tx_list_activate ? `background-color:${$props.activeColor}` : ""),
        d: common_vendor.o(($event) => $options.listClick(i), index)
      };
    }),
    Q: common_vendor.sr("chj-color-picker", "9a4b8c37-0"),
    R: common_vendor.o($options.subColor),
    S: common_vendor.o($options.cancelColorPicker),
    T: common_vendor.p({
    ...
    U: $data.textFocus,
    V: $data.text.length,
    W: common_vendor.o([($event) => $data.text = $event.detail.value, (...args) => $options.input && $options.input(...args)]),
    X: common_vendor.o(($event) => $data.textFocus = false),
    Y: $data.text,
    Z: $data.show,
